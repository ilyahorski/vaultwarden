...
export const ClassSelection: React.FC<ClassSelectionProps> = ({ onSelectClass, onParseCampaign }) => {
  ...
  return (
    <div className="absolute inset-0 z-50 bg-slate-900 flex flex-col items-center justify-center p-8 overflow-y-auto">
      <h2 className="text-4xl font-bold text-amber-500 mb-2">Новое Приключение</h2>
      <p className="text-slate-400 mb-6">Создайте своего героя</p>
      
      <div className="mb-8 w-full max-w-md">
          <label className="block text-slate-500 text-sm font-bold mb-2">Имя Героя</label>
          <input 
            type="text" 
            value={heroName}
            onChange={(e) => setHeroName(e.target.value)}
            className="w-full bg-slate-800 border border-slate-700 rounded p-3 text-white focus:border-amber-500 outline-none"
            placeholder="Введите имя..."
          />
      </div>

      <div className="mb-8 w-full max-w-md bg-slate-800 p-4 rounded-xl border border-slate-700">
          ...
          <div className="flex gap-4 mb-4">
              ...
              >
                  <Upload size={14}/> {campaign ? 'Файл выбран' : 'Загрузить'}
              </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl w-full">
        {(Object.keys(CLASSES) as ClassType[]).map(cls => (
          <button
            ...
            className="bg-slate-800 border-2 border-slate-700 hover:border-amber-500 hover:bg-slate-750 rounded-xl p-6 flex flex-col items-center text-center transition-all group relative overflow-hidden"
          >
            <div className="mb-4 p-4 bg-slate-900 rounded-full group-hover:scale-110 transition-transform relative z-10">
              {CLASSES[cls].icon}
            </div>
            <h3 className="text-2xl font-bold text-white mb-2 relative z-10">{CLASSES[cls].name}</h3>
            <p className="text-slate-400 text-sm mb-6 h-12 relative z-10">{CLASSES[cls].desc}</p>
            <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-sm w-full relative z-10">
